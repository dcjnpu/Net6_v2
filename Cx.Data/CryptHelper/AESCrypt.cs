...
namespace Cx.Data
{
    ...
    public class AESCrypt
    {
        ...
        private Aes _mAesCryptoServiceProvider;
        ...
        public AESCrypt()
        {
            _mAesCryptoServiceProvider = Aes.Create();
            _mContainKey = true;
        ...
        private string Encrypt(string sCrypto, byte[] key)
        {
            ...
            try
            {
                crypto = DataUtils.string2Byte(sCrypto);
                ...
                if (_mContainKey)
                {
                    sEncryped += DataUtils.byte2HexString(key);
                }
                sEncryped += DataUtils.byte2HexString(encrypted);
                return sEncryped;
            }
            catch (Exception ex)
            {
        ...
        public string Encrypt(string sCrypto, string sKey)
        {
            ...
            byte[] temp = DataUtils.string2Byte(sKey);
            if (temp.Length > key.Length)
            {
                _mMessage = "Key too long,need less than 32 Bytes key.";
                return RetError;
            }
            key = DataUtils.string2Byte(sKey.PadRight(key.Length));
        ...
        public string Decrypt(string sEncrypted)
        {
            ...
            key = DataUtils.hexString2Byte(s_key);
    ...
    public class DES3Encrypt
    {
        ...
        private static SymmetricAlgorithm mCSP = TripleDES.Create();
